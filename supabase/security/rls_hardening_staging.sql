-- RLS hardening script for staging first.
--
-- Do not run this directly in production until tested with:
-- 1. tyron@hvns.co.za
-- 2. a manager
-- 3. a director with no assigned jobs
-- 4. a director with one assigned job
-- 5. a normal staff user
--
-- Role model:
-- - tyron@hvns.co.za: unrestricted app data access.
-- - manager: full operational access.
-- - director: report/read access, plus assigned-job update access.
-- - staff: assigned-job access only.
...
using (
  ...
  or exists (
    ...
    where s.audit_id = audits.id
      and (
        s.assignee_email = public.current_user_email()
...
using (
  ...
  or exists (
    ...
    where ss.id = step_logs.subsection_id
      and (
        ss.assignee_email = public.current_user_email()
